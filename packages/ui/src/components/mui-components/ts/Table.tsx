...
const FilterDropdown = ({ label, value = [], options = [], onChange }: FilterTypes) => {
...
const SkeletonRows = ({ columns, rows = 5 }: skeletonType) =>
    Array.from({ length: rows }).map((_, rowIdx) => (
        <tr key={rowIdx} style={{ background: "#fff" }}>
            {columns.map((col: SkeletonColumn) => (
                <td
                    key={col.key}
                    style={{
                        padding: "12px 16px",
...
const DefaultCell = ({ value }: { value: unknown }) => (
...
export interface TableProps<T extends Record<string, unknown> = Record<string, unknown>> {
    ...
    showUnscannedFilter?: boolean;
    onUnscannedFilterChange?: (active: boolean) => void;
...
const DorametricsGenericTable = <T extends Record<string, unknown> = Record<string, unknown>>({
    data = [] as T[],
    ...
    isRowEmpty = (row: T) => (row as Record<string, unknown> & { _raw?: { has_data?: boolean } })?._raw?.has_data === false,
                            ...
                            {showSkeleton ? (
                                ...
                                rows.map((row, rowIdx) => {
                                    ...
                                    if (rowEmpty) {
                                        ...
                                        const isCollapsible = (col: ColumnDef<T>): boolean => col.isMetricColumn === true;

                                        const lastMiddleIdx = hasAction
                                            ? actionIdx - 1
                                            : columns.length - 1;

                                        const cellsToRender: Array<{ type: "message" } | { type: "normal"; idx: number }> = [];
                                        let messageSpan = 0;
